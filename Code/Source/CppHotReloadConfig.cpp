...
namespace CppHotReload
{
	...
	void LocateProjectPaths()
	{
		...
		//
		std::string boostrapFileContent = GetFileContent(BOOTSTRAP_FILENAME);
		{
			//
			// Project name
			//
			std::smatch matches;
			if (std::regex_search(boostrapFileContent, matches, std::regex(R"((sys_game_folder=)(\w+))")))
			{
				GAME_NAME = matches[2];
			}
			else
	...
	std::string CppHotReload::GetFileContent(const std::string& path)
	{
		...
		if (fileIO->Exists(path.c_str()))
		{
			...
			if (fileIO->Open(path.c_str(), AZ::IO::OpenMode::ModeRead | AZ::IO::OpenMode::ModeText, file) != AZ::IO::ResultCode::Success)
			{
				AZ_Error("C++ Hot Reload", false, "Impossible to open file: %s\n", path.c_str());
				abort();
			}
			...
			{
				fileIO->Close(file);
				AZ_Error("C++ Hot Reload", false, "%s is empty\n", path.c_str());
				abort();
			}

			fileContent.resize_no_construct(actualSize + 1);

			// Reading the entire bootstrap fileBinT
			if (fileIO->Read(file, fileContent.data(), actualSize) != AZ::IO::ResultCode::Success)
			{
				AZ_Error("C++ Hot Reload", false, "Something went wrong reading %s\n", path.c_str());
				abort();
			}
		...
		{
			AZ_Error("C++ Hot Reload", false, "%s doesn't exists\n", path.c_str());
			abort();
		}
	...
	{
		LyVersion version = LyVersion::Unknown;
		std::string settingsFileContent = GetFileContent(DEV_DIR + "/_WAF_/user_settings.options");
		{
			//
			// Project name
			//
			std::smatch matches;
			if (std::regex_search(settingsFileContent, matches, std::regex(R"(;version\s+=\s+(\w+\.\w+))")))
			{
				std::string strVersion = matches[1];
				if (strVersion == "1.24")
				{
					version = LyVersion::LY_1_24;
				}
				else if (strVersion == "1.23")
				{
					version = LyVersion::LY_1_23;
				}
				else if (strVersion == "1.22")
				{
					version = LyVersion::LY_1_22;
				}
			}
			else
			{
	...
	{
		if (lyVersion == LyVersion::LY_1_22)
		{
#if CPP_HOT_RELOAD_PLATFORM_WINDOWS
			//
			// Libraries directory
...
#error "Not yet supported"
		...
		else if (lyVersion == LyVersion::LY_1_23)
		{
#if CPP_HOT_RELOAD_PLATFORM_WINDOWS
			//
			// Libraries directory
...
#error "Not yet supported"
		...
		else if (lyVersion == LyVersion::LY_1_24)
		{
#if CPP_HOT_RELOAD_PLATFORM_WINDOWS
			//
			// Libraries directory
...
#error "Not yet supported"
	...
	std::string GetAdditionalLibraries(LyVersion lyVersion)
	{
		...
		{
#if CPP_HOT_RELOAD_PLATFORM_WINDOWS
			const std::string& additionalLibs = "AzGameFramework.lib AzFramework.lib GridMate.lib GridMateForTools.lib CryAction_AutoFlowNode.lib AzCore.lib lua.lib zstd.lib lz4.lib zlib.lib User32.lib Advapi32.lib PsAPI.lib User32.lib Advapi32.lib PsAPI.lib bcrypt.lib gdi32.lib ";
...
#error "Not yet supported"
		...
		{  
#if CPP_HOT_RELOAD_PLATFORM_WINDOWS
			const std::string& additionalLibs = "AzFramework.lib GridMate.lib GridMateForTools.lib AzCore.lib lua.lib zstd.lib lz4.lib zlib.lib User32.lib Advapi32.lib PsAPI.lib User32.lib Advapi32.lib PsAPI.lib bcrypt.lib ";
			return additionalLibs;
#elif CPP_HOT_RELOAD_PLATFORM_MAC  
#error "Not yet supported"   
#endif  
