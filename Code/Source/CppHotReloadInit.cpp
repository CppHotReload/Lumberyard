...
namespace CppHotReload
{
    ...
    void InitHotReload()
    {
		...
		//
		// Identify Lumberyard version
		//
		LyVersion lyVersion = GetLumberyardVersion();
		//
		// Libraries and their paths
		//
		const std::string& additionalLibPaths = GetAdditionalLibrarPaths(lyVersion);
		const std::string& additionalLibs     = GetAdditionalLibraries(lyVersion);
		//
		// Compile options
		//
	    ...
	    CPP_HOT_RELOAD_CALLBACK_LOG_SYSTEM 			  fnLogSystem)
	{
		...
		CppHotReload::ConfigureCallBacks(fnCallBackParsingFileProgress, fnCallBackBeginReload, fnCallBackEndReload, fnCallBackFilterToBuild, fnCallBackWillCompileFile, fnCallBackDidCompileFile, fnCallBackWillLinkFile, fnCallBackDidLinkFile,
